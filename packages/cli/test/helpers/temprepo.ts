...
/** The `@navbook/cli` package root, which holds the entry point under test. */
export const PACKAGE_ROOT = join(HERE, "..", "..");
/** The workspace root, which holds `doc/spec/fixtures`. */
...
/** The entry point under test, absolute so the cwd never matters. */
export const CLI_ENTRY = join(PACKAGE_ROOT, "src", "main.ts");

/** The command under test: `node src/main.ts`, or `$NAV_BIN` when set. */
export function navCommand(): string[] {
  ...
  return [process.execPath, CLI_ENTRY];
}
