...
- **Pull requests**, read, reviewed and edited: listing with that same filter, the all-branches toggle and an "awaiting me" one, revisions, comments and reviews bound to a revision, the people asked to review with what each has said about the latest revision and the decision that adds up to, and every other field `updatePr` takes — title, description, labels, assignees, features, milestone — each editable in place as an issue's is, minus `rank` and `deadline`, which are an issue's alone (spec 02 §2.5). The refusal to write on a branch the server does not hold is shown with the branch it names, and withdraws every one of those editors rather than only the one that was refused: the refusal is about the branch, so it is the same answer for all of them.
