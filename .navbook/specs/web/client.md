...
- **Pull requests**, read, reviewed and edited: listing with that same filter, the all-branches toggle and an "awaiting me" one, revisions, comments and reviews bound to a revision, the people asked to review with what each has said about the latest revision and the decision that adds up to, and every other field `updatePr` takes — title, description, labels, assignees, features, milestone — each editable in place as an issue's is, minus `rank` and `deadline`, which are an issue's alone (spec 02 §2.5). The page has the three tabs every forge has, kept in the address as `?tab=`: **Conversation** is the description and the discussion; **Commits** lists what the latest revision introduces, `base..head` oldest first, with subject, author, date and short hash; **Changes** is the diff of that revision against its merge base, file by file, with a summary line, a file list with anchors, status letters, counts, hunk headers, line numbers on both sides and the changed span marked within a replaced line. Neither of the two is asked for until its tab is opened, so the page opens as fast as it did with one. Files come in git's order with the tracker's own last; a file the server withheld for size shows its counts and a "Load diff" button, a binary file says so, and files below the fold are rendered progressively in idle time under `content-visibility: auto`, so a five-thousand-line diff paints in well under a second. The refusal to write on a branch the server does not hold is shown with the branch it names, and withdraws every one of those editors rather than only the one that was refused: the refusal is about the branch, so it is the same answer for all of them.
