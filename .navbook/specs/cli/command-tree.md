...
- **Query grammar** — `status:`, `label:`, `assignee:`, `author:`, `milestone:`, `feature:` and bare words, plus `reviewer:`, `review:` and `awaiting:` on pull requests and `deadline:overdue|none` on issues, each refused on the other noun rather than matching nothing. Single-valued keys OR their terms; multi-valued keys AND them. The `status:open` default belongs to the CLI's `list` verbs alone, and so does the day `overdue` is judged against: it comes from the workspace, so `NAV_NOW` reaches it.
- **Listing order** — `nav issue list --sort priority|deadline|newest`, defaulting to newest, which is §4.2's order. It orders `--json` as it orders the table, and `rank` and `deadline` columns appear when any issue listed carries one, as `labels` does. Sorting is the front end's own reading: core hands every listing over newest first and takes no sort argument.
...
- `packages/cli/src/program.ts` (the tree), `commands/`, `install/`, `render/`, `sort.ts`
