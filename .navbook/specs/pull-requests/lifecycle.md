...
- **Open** — `nav pr open [--target BRANCH] [--title T] [--draft]` on the current branch, pinning one revision. The title defaults to the last commit's subject.
- **Update** — `nav pr update` appends a revision for the current `HEAD`, refusing when it equals the last recorded head.
- **Review** — `nav pr review --approve|--request-changes`, bound to the latest revision unless `--revision` says otherwise, with `--file` and `--line` for an inline anchor. A verdict on one revision carries nothing to the next (spec 03 §3.5).
...
The API reads pull requests and takes comments and reviews, and refuses a comment on a pull request whose branch it does not serve, naming the branch. Opening, updating, merging and deleting are checkout-centric and are not exposed (spec 06 §6.3).

## Where it lives

- `packages/core/src/ops/pr.ts`, `packages/core/src/git/{refscan,merge}.ts`
